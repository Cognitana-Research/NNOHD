...
parser.add_argument('-w', 
                    '--writerankings', 
                    action='store_true',
                    help='Write the outlier ranking(s) to file(s). For each \
                    method, a separate file is created that contains the \
                    outlier rankings for each data object.')
...
if args.method in ('ae', 'all'):
    ...
    if save_rankings:
        with open('ae_{}epochs_{}encodingfactor_{}.csv'.format(
            ...
            aefile.write('Data object ID;Outlier score\n')
            counter = 0
    ...
    if save_rankings:
        with open('rbm_{}epochs_{}hidden_{}.csv'.format(
            ...
            # Save outlier ranking descending
            rbmfile.write('Data object number;Outlier score\n')
            counter = 0
            for value in rbm.fe:
                rbmfile.write('{};{}\n'.format(counter, value))
                counter += 1
    ...
    if save_rankings:
        with open('som_{}epochs_{}grid_{}.csv'.format(
            ...
            # Save outlier ranking descending
            somfile.write('Data object number;Outlier score\n')
            counter = 0
            for value in som.qe:
                somfile.write('{};{}\n'.format(counter, value))
                counter += 1
